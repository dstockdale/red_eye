      ...
      <:item heading="Authenticator App">
        <%= if is_nil(@current_user.totp_confirmed_at) do %>
          <div class="leading-8 text-md text-zinc-700" id="app-auth-device-intro">
            ...
            >
              Setup Authentication App <.icon name={:play} solid class="w-3 h-3" />
            </.button>
          </div>
        ...
        <% else %>
          ...
          <.form for={@reset_otp_form} phx-submit="reset_otp_secret" class="mt-6">
            <.button
              icon={:trash}
              color="secondary"
              label="Reset Authentication App"
              size="xs"
              data-confirm="Are  you sure? You'll have to setup a new Auth application"
            />
          </.form>
        <% end %>
      </:item>
